...
mods: looks for actions and parmaters.  For instance
  "billing" does a cost summary.  If collowed by a parameter, it totals that number of days.  Otherwise it defaults to 1 day.
...
def lambda_handler(event, context):
    ...
    command_text_split = command_text.split(' ')
    action = command_text_split[0]
    if len(command_text_split) == 1:
        days = 1
    else:
        days = int(command_text_split[1])
    ...
    if action == "billing":
        ...
        print('\t'.join(['TimePeriod', 'LinkedAccount', 'Service', 'Amount', 'Unit']))
        for result_by_time in results:
            for group in result_by_time['Groups']:
                amount = group['Metrics']['UnblendedCost']['Amount']
                total = total + float(amount)
                unit = group['Metrics']['UnblendedCost']['Unit']
                print(result_by_time['TimePeriod']['Start'], '\t', '\t'.join(group['Keys']), '\t', amount, '\t', unit)
        return respond(None, "Cost for the last %i days.  Total= $%5.2f %s" % (days, round(total, 2), unit))
